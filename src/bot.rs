use crate::{
    ...
    autosell::{run_after_mint, simulate_auto_sell},
    config::{
...
async fn simulate_configured_auto_sell(
    config: &MintConfig,
    rpc: &RpcClients,
    wallet: &LoadedWallet,
    ...
    let token_id = config
        ...
        .ok_or_else(|| {
            BotError::Config(
                "auto_sell.dry_run_token_id is required to validate auto-sell in simulate/--dry-run"
                    .to_string(),
            )
        })?
        .parse::<U256>()
        .map_err(|_| BotError::Config("auto_sell.dry_run_token_id is invalid".to_string()))?;
    let mut mint_gas_amount = U256::from(prepared.gas_limit)
        .checked_mul(U256::from(prepared.fee_cap))
        .ok_or_else(|| BotError::Transaction("dry-run mint gas cost overflowed".to_string()))?;
    if config.chain_id == crate::config::INK_MAINNET_CHAIN_ID {
        mint_gas_amount = mint_gas_amount
            .checked_add(
                rpc.ink_extra_fee_reserve(prepared.calldata.len(), prepared.gas_limit)
                    .await?,
            )
            .ok_or_else(|| BotError::Transaction("dry-run mint fees overflowed".into()))?;
    }
    let client = OpenSeaClient::from_env()?;
    let _ = simulate_auto_sell(
        config,
        rpc,
        wallet,
        &client,
...
async fn execute_transaction(
        ...
        && let Some(receipt) = receipt
    {
        let client = OpenSeaClient::from_env()?;
        let summary =
            run_after_mint(config, rpc, wallet, &client, &receipt, prepared.mint_value).await?;
        println!(
            "Auto-sell complete: sold {}, skipped {}",
            summary.sold.len(),
            summary.skipped.len()
        );
    }
...
) -> Result<(MonitorOutcome, Option<TransactionReceipt>)> {
    ...
    loop {
        tokio::select! {
            _ = &mut shutdown => {
                ...
                return Ok((MonitorOutcome::Shutdown, None));
            }
        ...
        if let Some((mined_hash, mut receipt)) = mined {
            if receipt.block_number.is_none() || receipt.block_hash.is_none() {
                tracing::warn!(%mined_hash, "receipt is missing inclusion data; continuing monitoring");
                continue;
            }
                ...
                && let Some(receipt_block) = receipt.block_number
            {
                ...
                match rpc.transaction_receipt(mined_hash).await {
                    Ok(Some(canonical_receipt)) => {
                        if canonical_receipt.block_hash != receipt.block_hash
                            || canonical_receipt.block_number != receipt.block_number
                        {
                            continue;
                        }
            ...
            if receipt.status() {
                ...
                return Ok((MonitorOutcome::Done, Some(receipt)));
            }
                ...
                && mint_was_closed_before_receipt(rpc, &request, receipt.block_number).await
            {
                ...
                return Ok((MonitorOutcome::Retry, None));
            }
                ...
                if !bumped {
                    ...
                    match rpc.broadcast_raw(signed.encoded_2718()).await {
                        ...
                        Err(BotError::BroadcastOutcomeUnknown {
                            ...
                            // It may be the replacement that mines. Keep both
                            // hashes observable even when acknowledgement was lost.
                            candidate_hashes.push(replacement_hash);
                            request = replacement;
                            hash = replacement_hash;
                            replacements = config.replacement.max_attempts;
...
mod tests {
    ...
    use super::{
        AtomicBotState, B256, BotState, Duration, LoadedWallet, PreparedTransaction,
        ...
        is_opensea_stage_not_active_rejection, monitor_receipt, next_opensea_stage,
    ...
    fn review_wallet() -> LoadedWallet {
        let signer =
            alloy::signers::local::PrivateKeySigner::from_bytes(&B256::with_last_byte(1)).unwrap();
        LoadedWallet {
            address: signer.address(),
            wallet: alloy::network::EthereumWallet::new(signer),
        }
    ...
    fn review_prepared(config: &MintConfig, wallet: &LoadedWallet) -> PreparedTransaction {
        let calldata = encode_mint(&config.mint, config.quantity, wallet.address, None)
            .unwrap()
            .bytes;
        let request = TransactionRequest::default()
            .with_from(wallet.address)
    ...
    #[tokio::test]
    async fn re_mined_receipt_waits_for_confirmations_at_its_new_height() {
        use std::sync::atomic::{AtomicUsize, Ordering};
        static HEADS: AtomicUsize = AtomicUsize::new(0);
        ...
        let (rpc, server) =
            crate::rpc::tests::mock_rpc(|request| match request["method"].as_str().unwrap() {
                "eth_blockNumber" => {
                    let count = HEADS.fetch_add(1, Ordering::SeqCst);
                    serde_json::json!(if count < 3 { "0x66" } else { "0x67" })
                }
                "eth_getTransactionReceipt" => {
                    let count = RECEIPTS.fetch_add(1, Ordering::SeqCst);
                    review_receipt(B256::ZERO, if count == 0 { 100 } else { 101 })
                }
        ...
        let wallet = review_wallet();
        let prepared = review_prepared(&config, &wallet);
        let state = AtomicBotState::new(BotState::Submitted);
        let (_, receipt) = tokio::time::timeout(
        ...
        .unwrap()
        .unwrap();
        assert_eq!(receipt.unwrap().block_number, Some(101));
        assert!(HEADS.load(Ordering::SeqCst) >= 4);
        server.abort();
    }

    #[tokio::test]
    async fn replacement_with_lost_acknowledgement_is_still_monitored() {
        use std::sync::Mutex;
        static REPLACEMENT: Mutex<Option<B256>> = Mutex::new(None);
        *REPLACEMENT.lock().unwrap() = None;
        let (rpc, server) =
            crate::rpc::tests::mock_rpc(|request| match request["method"].as_str().unwrap() {
                "eth_blockNumber" => {
                    static HEAD: std::sync::atomic::AtomicU64 =
                        std::sync::atomic::AtomicU64::new(100);
                    serde_json::json!(format!(
                        "0x{:x}",
                        HEAD.fetch_add(1, std::sync::atomic::Ordering::SeqCst)
                    ))
                }
                "eth_sendRawTransaction" => {
                    ...
                    .unwrap();
                    *REPLACEMENT.lock().unwrap() = Some(alloy::primitives::keccak256(raw));
                    serde_json::Value::Null
                }
                "eth_getTransactionReceipt" => {
                    let requested: B256 = request["params"][0].as_str().unwrap().parse().unwrap();
                    if *REPLACEMENT.lock().unwrap() == Some(requested) {
                        review_receipt(requested, 101)
                    } else {
                        serde_json::Value::Null
                    }
                }
        ...
        config.replacement.after_blocks = 1;
        let wallet = review_wallet();
        let prepared = review_prepared(&config, &wallet);
        let state = AtomicBotState::new(BotState::Submitted);
        let (_, receipt) = tokio::time::timeout(
        ...
        .unwrap();
        assert_eq!(
            Some(receipt.unwrap().transaction_hash),
