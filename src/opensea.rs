...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct OpenSeaFee {
    /// Fee in basis points. For example, 250 means 2.5%.
...
#[derive(Debug, Clone)]
pub struct OpenSeaOfferFulfillment {
    pub protocol: String,
    pub transaction: OpenSeaFulfillmentTransaction,
    pub orders: Vec<Value>,
}

#[derive(Debug, Clone)]
pub struct OpenSeaFulfillmentTransaction {
    pub function: String,
    pub chain: u64,
    pub to: Address,
    pub value: U256,
...
#[derive(Debug, Serialize)]
struct FulfillOfferRequest {
    offer: FulfillOfferReference,
    fulfiller: FulfillerReference,
    consideration: OfferConsideration,
    units_to_fill: u64,
    include_optional_creator_fees: bool,
}

#[derive(Debug, Serialize)]
struct FulfillOfferReference {
    hash: String,
    chain: String,
...
#[derive(Debug, Serialize)]
struct FulfillerReference {
    address: String,
}

#[derive(Debug, Serialize)]
struct OfferConsideration {
...
impl OpenSeaClient {
    ...
    pub async fn get_best_offer(
        ...
        collection_slug: &str,
        token_id: U256,
    ) -> Result<Option<OpenSeaOffer>> {
        let url =
            format!("{OPENSEA_API_BASE}/offers/collection/{collection_slug}/nfts/{token_id}/best");
        let response = self
            .client
            .get(url)
            .header("X-API-KEY", self.api_key.as_str())
        ...
        let body: Value =
            read_json_response(response, "OpenSea returned invalid best-offer data").await?;
        // The NFT-specific endpoint is already scoped to `token_id`. Some
        // collection/criteria offers omit `asset.identifier` (or return it
        // as null), so pass the requested ID through as the authoritative
        // value when parsing the response.
        parse_best_offer(&body, token_id).map(Some)
    }

    pub async fn get_collection_fees(&self, collection_slug: &str) -> Result<Vec<OpenSeaFee>> {
        let url = format!("{OPENSEA_API_BASE}/collections/{collection_slug}");
        let response = self
            .client
            .get(url)
            .header("X-API-KEY", self.api_key.as_str())
        ...
        let body: Value =
            read_json_response(response, "OpenSea returned invalid collection fee data").await?;
        parse_collection_fees(&body)
    }

    pub async fn build_offer_fulfillment(
        &self,
        offer: &OpenSeaOffer,
        fulfiller: Address,
        include_optional_creator_fees: bool,
    ) -> Result<OpenSeaOfferFulfillment> {
        let body = FulfillOfferRequest {
            offer: FulfillOfferReference {
                hash: offer.order_hash.clone(),
                chain: offer.chain.clone(),
                protocol_address: format!("{:#x}", offer.protocol_address),
            },
            fulfiller: FulfillerReference {
                address: format!("{fulfiller:#x}"),
            },
            consideration: OfferConsideration {
                asset_contract_address: format!("{:#x}", offer.contract_address),
        ...
        let response = self
            .client
            .post(format!("{OPENSEA_API_BASE}/offers/fulfillment_data"))
            .header("X-API-KEY", self.api_key.as_str())
            .json(&body)
            .send()
            .await
        ...
        let body: Value =
            read_json_response(response, "OpenSea returned invalid offer fulfillment data").await?;
        parse_offer_fulfillment(&body)
    }
...
fn parse_best_offer(body: &Value, requested_token_id: U256) -> Result<OpenSeaOffer> {
    let order_hash = body
        .get("order_hash")
        .and_then(Value::as_str)
        .ok_or_else(|| BotError::Transaction("OpenSea offer has no order hash".to_string()))?
        .to_string();
    let chain = body
        .get("chain")
        .and_then(Value::as_str)
        .ok_or_else(|| BotError::Transaction("OpenSea offer has no chain".to_string()))?
        .to_string();
    let protocol_address = body
        .get("protocol_address")
        .and_then(Value::as_str)
        .ok_or_else(|| BotError::Transaction("OpenSea offer has no protocol address".to_string()))?
        .parse()
        .map_err(|_| {
            BotError::Transaction("OpenSea offer has an invalid protocol address".into())
        })?;
    let asset = body
        .get("asset")
        .ok_or_else(|| BotError::Transaction("OpenSea offer has no asset".to_string()))?;
    let contract_address = asset
        .get("contract")
        .and_then(Value::as_str)
        .ok_or_else(|| BotError::Transaction("OpenSea offer has no NFT contract".to_string()))?
        .parse()
        .map_err(|_| BotError::Transaction("OpenSea offer has an invalid NFT contract".into()))?;
    let token_id = match asset.get("identifier") {
        // Criteria offers may not carry a concrete identifier in the asset
        // object. The request URL is token-specific, so use that ID.
        None | Some(Value::Null) => requested_token_id,
        Some(value) => {
            let parsed = parse_json_u256(value, "token ID")?;
            if parsed != requested_token_id {
                return Err(BotError::Transaction(
                    "OpenSea offer token ID does not match requested NFT".to_string(),
                ));
            }
            parsed
        }
    ...
    let price = body
        .get("price")
        .ok_or_else(|| BotError::Transaction("OpenSea offer has no price".to_string()))?;
    // Some older responses wrapped the price in `price.current`; the current
    // best-offer endpoint returns the object directly. Accept both shapes so
    // a cached/API-proxy response cannot silently disable auto-sell.
    let price = price.get("current").unwrap_or(price);
    let currency = price
        .get("currency")
        .and_then(Value::as_str)
        .ok_or_else(|| BotError::Transaction("OpenSea offer has no currency".to_string()))?
        .to_string();
    let value = parse_json_u256(
        price
            .get("value")
            .ok_or_else(|| BotError::Transaction("OpenSea offer has no price value".to_string()))?,
        "offer value",
    )?;
    let decimals = price
        .get("decimals")
        .and_then(Value::as_u64)
        .ok_or_else(|| BotError::Transaction("OpenSea offer has no currency decimals".into()))?;
    let decimals = u8::try_from(decimals)
        .map_err(|_| BotError::Transaction("OpenSea offer decimals are invalid".into()))?;
    if decimals > 36 {
        return Err(BotError::Transaction(
            "OpenSea offer decimals exceed the supported precision".into(),
        ));
    }
    let status = body
        .get("status")
...
fn parse_collection_fees(body: &Value) -> Result<Vec<OpenSeaFee>> {
    let Some(fees) = body.get("fees").and_then(Value::as_array) else {
        return Ok(Vec::new());
    };
    fees.iter()
        .map(|fee| {
            let raw = fee
                .get("fee")
                .ok_or_else(|| BotError::Transaction("OpenSea fee has no value".to_string()))?;
            let basis_points = parse_fee_bps(raw)?;
            let required = fee
                .get("required")
...
fn parse_fee_bps(value: &Value) -> Result<u64> {
    let text = match value {
        Value::String(value) => value.clone(),
        Value::Number(value) => value.to_string(),
        _ => {
            return Err(BotError::Transaction(
                "OpenSea fee value is not numeric".to_string(),
            ));
        }
    };
    let value = text
        .parse::<f64>()
        .map_err(|_| BotError::Transaction(format!("OpenSea fee value `{text}` is invalid")))?;
    if !(value.is_finite() && value >= 0.0) {
        return Err(BotError::Transaction(
            "OpenSea fee value must be non-negative".to_string(),
        ));
    }
    let bps = if value <= 100.0 {
        (value * 100.0).round()
    } else {
        value.round()
    };
    if bps > 10_000.0 {
        return Err(BotError::Transaction(
            "OpenSea fee value exceeds 100%".to_string(),
        ));
    }
    Ok(bps as u64)
}

fn parse_offer_fulfillment(body: &Value) -> Result<OpenSeaOfferFulfillment> {
    let protocol = body
        .get("protocol")
        .and_then(Value::as_str)
        .ok_or_else(|| BotError::Transaction("OpenSea fulfillment has no protocol".to_string()))?
        .to_string();
    let transaction = body
        .get("fulfillment_data")
        .and_then(|data| data.get("transaction"))
        .ok_or_else(|| {
            BotError::Transaction("OpenSea fulfillment has no transaction".to_string())
        })?;
    let orders = body
        .get("fulfillment_data")
    ...
    let function = transaction
        ...
        .and_then(Value::as_str)
        .ok_or_else(|| {
            BotError::Transaction("OpenSea fulfillment has no transaction function".to_string())
        })?
        .to_string();
    let chain = parse_json_u64(
        transaction
            .get("chain")
            .ok_or_else(|| BotError::Transaction("OpenSea fulfillment has no chain".to_string()))?,
        "fulfillment chain",
    )?;
    let to = transaction
        .get("to")
        .and_then(Value::as_str)
        .ok_or_else(|| BotError::Transaction("OpenSea fulfillment has no target".to_string()))?
        .parse()
        .map_err(|_| BotError::Transaction("OpenSea fulfillment target is invalid".into()))?;
    let value = parse_json_u256(
        transaction
            .get("value")
            .ok_or_else(|| BotError::Transaction("OpenSea fulfillment has no value".to_string()))?,
        "fulfillment value",
    )?;
    let input_data = transaction
        .get("input_data")
        .cloned()
        .ok_or_else(|| BotError::Transaction("OpenSea fulfillment has no input data".into()))?;
    Ok(OpenSeaOfferFulfillment {
        protocol,
...
fn parse_json_u256(value: &Value, field: &str) -> Result<U256> {
    let text = match value {
        Value::String(value) => value.clone(),
        Value::Number(value) => value.to_string(),
        _ => {
            return Err(BotError::Transaction(format!(
                "OpenSea {field} is not numeric"
            )));
        }
    };
    text.parse::<U256>()
        .map_err(|_| BotError::Transaction(format!("OpenSea {field} is invalid")))
}

fn parse_json_u64(value: &Value, field: &str) -> Result<u64> {
    let text = match value {
        Value::String(value) => value.clone(),
        Value::Number(value) => value.to_string(),
        _ => {
...
mod tests {
    use super::{
        ...
        opensea_chain_slug, parse_best_offer, parse_drop_details, parse_stage, parse_u256,
    ...
    #[test]
    fn uses_requested_token_id_when_opensea_omits_identifier() {
        let requested_token_id = U256::from(42);
        let offer = parse_best_offer(
            &json!({
                "order_hash": "0x01",
                "chain": "robinhood",
        ...
        assert_eq!(offer.token_id, requested_token_id);
    }

    #[test]
    fn rejects_offer_for_a_different_token_id() {
        let error = parse_best_offer(
            &json!({
                "order_hash": "0x01",
                "chain": "robinhood",
                "protocol_address": "0x0000000000000000000000000000000000000001",
        ...
        assert!(error.to_string().contains("does not match requested NFT"));
    }
