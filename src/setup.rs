use crate::{
    config::{
        AutoSellConfig, GasConfig, HYPEREVM_DEFAULT_GAS_LIMIT,
        HYPEREVM_DEFAULT_MAX_GAS_COST_NATIVE, HYPEREVM_MAINNET_CHAIN_ID, INK_DEFAULT_GAS_LIMIT,
        INK_DEFAULT_MAX_GAS_COST_NATIVE, INK_MAINNET_CHAIN_ID, MintCallConfig, MintConfig,
        MintTrigger, NonceStrategy, OpenSeaExecutionMode, ROBINHOOD_DEFAULT_GAS_LIMIT,
...
fn prompt_config(allow_manual: bool) -> Result<MintConfig> {
    ...
    if let Some(opensea_drop_slug) = opensea_drop_slug {
        ...
        let auto_sell = prompt_auto_sell_config(Some(&opensea_drop_slug), chain_id)?;
        let config = MintConfig {
            ...
            trigger: MintTrigger::BlockTimestamp {
                // OpenSea owns stage selection. Start with the first active or
                // upcoming stage and advance automatically when needed.
                timestamp: 0,
            },
            gas: GasConfig {
    ...
    let auto_sell = prompt_auto_sell_config(None, chain_id)?;
    let config = MintConfig {
        name,
        chain_id,
        native_currency: (chain_id == HYPEREVM_MAINNET_CHAIN_ID).then(|| "HYPE".to_string()),
...
fn prompt_auto_sell_config(default_slug: Option<&str>, chain_id: u64) -> Result<AutoSellConfig> {
    if !ask_yes_no("Enable post-mint OpenSea auto-sell", false)? {
        return Ok(AutoSellConfig::default());
    }
    let slug = if let Some(slug) = default_slug.filter(|value| !value.trim().is_empty()) {
        println!("OpenSea collection slug: {slug} (reused from mint setup)");
        slug.to_string()
    } else {
        ask("OpenSea collection slug", "")?
    };
    let opensea_chain = ask("OpenSea chain slug (blank for built-in mapping)", "")?;
    let currency = ask("Offer currency symbol", "WETH")?;
    let default_token = if chain_id == 1 && currency.eq_ignore_ascii_case("WETH") {
        "0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2"
    } else {
        ""
    };
    let token = ask(
        "Trusted offer ERC-20 contract on the selected chain",
        default_token,
    )?;
    let currency_token_addresses = std::collections::BTreeMap::from([(currency, token)]);
    let min_profit_usd = ask("Minimum net profit in USD", "25")?;
    let offer_wait_seconds = ask("Offer/indexing wait timeout (seconds)", "180")?
        .parse()
        .map_err(|err| BotError::Config(format!("invalid offer wait timeout: {err}")))?;
    let offer_poll_seconds = ask("Offer polling interval (seconds)", "5")?
        .parse()
        .map_err(|err| BotError::Config(format!("invalid offer polling interval: {err}")))?;
    let receipt_timeout_seconds = ask("Approval/sale receipt timeout (seconds)", "180")?
        .parse()
        .map_err(|err| BotError::Config(format!("invalid receipt timeout: {err}")))?;
    let max_sell_gas_cost_native = ask("Maximum sell gas cost (native currency)", "0.001")?;
    let dry_run_token_id = ask("Existing token ID for auto-sell dry-run (optional)", "")?;
    Ok(AutoSellConfig {
        enabled: true,
        collection_slug: Some(slug),
        opensea_chain: (!opensea_chain.trim().is_empty()).then_some(opensea_chain),
        min_profit_usd,
        offer_wait_seconds,
        offer_poll_seconds,
        receipt_timeout_seconds,
        // Keep optional creator fees disabled by default. Advanced users can
        // still enable them by editing the generated JSON configuration.
        include_optional_creator_fees: false,
        max_sell_gas_cost_native: Some(max_sell_gas_cost_native),
        // Require a current USD conversion before any automatic sale.
        require_usd_price: true,
        currency_usd_prices: Default::default(),
        currency_token_addresses,
        dry_run_token_id: (!dry_run_token_id.trim().is_empty()).then_some(dry_run_token_id),
    })
}
...
fn ask(label: &str, default: &str) -> Result<String> {
    ...
    if io::stdin().read_line(&mut line)? == 0 {
        return Err(BotError::Config(
            "setup input ended before completion".into(),
        ));
    }
    let value = line.trim();
