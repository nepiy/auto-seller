...
impl RpcClients {
    ...
    /// Recover L1 data charges that the generic Ethereum receipt omits, and
    /// query the operator fee using the oracle state in the inclusion block.
    pub async fn ink_receipt_extra_fee(&self, receipt: &TransactionReceipt) -> Result<U256> {
        use alloy::{network::TransactionBuilder, primitives::address, sol_types::SolCall};
        alloy::sol! { function getOperatorFee(uint256 gasUsed) external view returns (uint256); }
        let hash = receipt.transaction_hash;
        let block_hash = receipt
            .block_hash
            .ok_or_else(|| BotError::Rpc("receipt has no block hash".into()))?;
        let receipts = self
            .read_all(
                "eth_getTransactionReceipt fees",
                move |provider| async move {
                    provider
                        .client()
                        .request::<_, serde_json::Value>("eth_getTransactionReceipt", (hash,))
                        .await
                },
            )
            .await?;
        let mut l1 = U256::ZERO;
        for raw in receipts {
            let matches = raw
                .get("blockHash")
                .and_then(|v| v.as_str())
            ...
            if !matches || fee.is_none() {
                return Err(BotError::Rpc(
                    "Ink receipt is missing canonical L1 fee data".into(),
                ));
            }
            l1 = l1.max(fee.unwrap_or_default());
        }
        let output = self
            .call_at(
                TransactionRequest::default()
                    .with_to(address!("420000000000000000000000000000000000000F"))
                    .with_input(
                        getOperatorFeeCall {
                            gasUsed: U256::from(receipt.gas_used),
                        }
                        .abi_encode(),
                    ),
            ...
            .await?;
        let operator = getOperatorFeeCall::abi_decode_returns(&output)
            .map_err(|_| BotError::Rpc("invalid historical Ink operator fee".into()))?;
        l1.checked_add(operator)
            .ok_or_else(|| BotError::Transaction("receipt fees overflowed".into()))
    }
    ...
    pub async fn validate_chain(&mut self, config: &MintConfig) -> Result<()> {
        ...
        // transaction sooner than an idle HTTP connection; keep HTTP as
        // concurrent fallbacks.
        ...
        while let Some(joined) = tasks.next().await {
            ...
            match result {
                ...
                Ok(Err(err))
                    if err
                        .as_error_resp()
                        .is_some_and(|response| is_known_transaction(&response.message)) =>
                {
                    let elapsed = started.elapsed();
                ...
                Ok(Err(err)) => {
                    // A dropped connection or malformed response can happen
                    // after the node accepted the signed transaction.
                    ambiguous |= !err.is_error_resp();
...
fn register_http_endpoint(seen: &mut HashSet<Url>, name: &str, url: &str) -> Result<bool> {
    ...
    // provider routes remain independent endpoints.
...
fn is_known_transaction(message: &str) -> bool {
    ...
    .iter()
    .any(|needle| message.starts_with(needle))
}
...
pub(crate) mod tests {
    ...
    fn recognizes_known_transaction_responses() {
        ...
        assert!(!is_known_transaction("unknown transaction: 0xabc"));
        assert!(!is_known_transaction(
            "transport response mentioned already known"
        ));
    }
    ...
    fn requires_encrypted_rpc_transport_except_on_loopback() {
        ...
        assert!(validate_rpc_url("WS_RPC_URL", "ws://[::1]:8545", true).is_ok());
    }
    ...
    #[tokio::test]
    async fn malformed_broadcast_acknowledgement_preserves_the_signed_hash() {
        let (rpc, server) = mock_rpc(|_| serde_json::Value::Null).await;
        let raw = vec![1, 2, 3];
        let expected = keccak256(&raw);
        assert!(matches!(rpc.broadcast_raw(raw).await,
            Err(BotError::BroadcastOutcomeUnknown { hash }) if hash == expected));
        server.abort();
    }

    #[tokio::test]
    async fn ink_confirmed_fees_use_receipt_l1_data_and_historical_operator_fee() {
        let block_hash = B256::repeat_byte(1);
        let receipt: TransactionReceipt = serde_json::from_value(serde_json::json!({
            "transactionHash":B256::ZERO, "blockHash":block_hash, "blockNumber":"0x64",
            "from":Address::ZERO, "gasUsed":"0x1", "effectiveGasPrice":"0x1",
            "cumulativeGasUsed":"0x1", "status":"0x1", "logs":[],
            "logsBloom":format!("0x{}", "00".repeat(256)), "type":"0x2"
        }))
        .unwrap();
        let (rpc, server) = mock_rpc(|request| {
            if request["method"] == "eth_getTransactionReceipt" {
                serde_json::json!({"blockHash":B256::repeat_byte(1), "l1Fee":"0x64"})
            } else {
                let expected = serde_json::json!(B256::repeat_byte(1));
                assert!(
                    request["params"][1] == expected
                        || request["params"][1]["blockHash"] == expected
                );
                serde_json::json!(format!("0x{:064x}", 20))
            }
        })
        .await;
        assert_eq!(
            rpc.ink_receipt_extra_fee(&receipt).await.unwrap(),
            U256::from(120)
        );
        server.abort();
    }
