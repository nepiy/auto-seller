...
| --- | --- | --- |
| Workflow | Prepare, trigger, submit, and confirm an NFT mint. | Continue after a successful mint with optional OpenSea offer lookup and fulfillment. |
| NFT detection | Report the mint transaction result. | Extract ERC-721 and ERC-1155 token IDs from the mint receipt and evaluate each token for sale. |
| Sale decisions | No post-mint selling strategy. | Compare estimated net USD profit against `auto_sell.min_profit_usd`, accounting for allocated mint payment, mint gas, encoded marketplace/creator fees, approval gas, and sale gas. |
| Currency pricing | No auto-sell USD valuation layer. | Use a fresh ETH/WETH price snapshot or explicit currency prices, with trusted ERC-20 address and decimal checks. |
...
The Cargo package and executable retain the name `nft-mint-bot`, so the commands below still use that name. The existing setup and runtime guide follows.
...
```bash
git clone https://github.com/nepiy/auto-seller.git
cd auto-seller
```

#### Alternative: download a ZIP

Open [the Auto Seller repository](https://github.com/nepiy/auto-seller), select **Code → Download ZIP**, extract it, and open a terminal in the extracted `auto-seller-main` directory.

On macOS or Linux, the equivalent terminal commands are:

```bash
curl -L https://github.com/nepiy/auto-seller/archive/refs/heads/main.zip -o auto-seller.zip
unzip auto-seller.zip
cd auto-seller-main
```
...
```powershell
Invoke-WebRequest https://github.com/nepiy/auto-seller/archive/refs/heads/main.zip -OutFile auto-seller.zip
Expand-Archive .\auto-seller.zip -DestinationPath .
Set-Location .\auto-seller-main
```
...
Use **normal** for the first run and for ordinary mints. It performs fresh gas simulation, balance checks, and just-in-time nonce selection. Without a fixed gas limit, OpenSea gas estimation and the final budget check are deferred until eligible SeaDrop calldata is available; the placeholder cannot be signed. Use **aggressive** only for a tested contract when trigger latency is critical and you already know a safe fixed gas limit and fee budget. Aggressive mode skips live gas simulation and normally uses a cached balance; it still refreshes the nonce under the wallet lock before signing. Ink still performs fresh surcharge and balance checks before signing in either mode. A wrong gas limit or changed contract state can still produce a gas-paying revert.
...
When the dry run reaches its trigger, it builds and simulates the mint transaction without signing or broadcasting. With auto-sell enabled, it also validates the offer and fulfillment workflow using `auto_sell.dry_run_token_id`. For a saved JSON configuration, `simulate --config ...` runs this preflight immediately; it may report that no stage is active if run outside a stage window.
...
For direct mode, leave the drop-slug prompt blank when no OpenSea key is configured, or type `direct` explicitly. Post-mint auto-sell requires `OPENSEA_API_KEY` even when the mint itself uses direct mode.
...
Auto-sell accepts only zero-value calls to canonical Seaport 1.6. It checks the exact ABI, verifies the deployed target, and permits only one unit of the selected NFT plus fees in the trusted offer token. Matching supports one buyer order and one seller order, with every asset accounted for. Unsupported payloads stop before approval.

Configure `auto_sell.currency_token_addresses` with the ERC-20 addresses you trust **on the selected chain**. Ethereum WETH has a built-in default; other tokens and chains require an entry. For example, on Ethereum:
...
The setup wizard asks for this address when enabling auto-sell. Currency decimals are checked against that contract. `currency_usd_prices` still supplies the USD valuation; it does not establish token identity. ETH and WETH share a single price snapshot and conflicting explicit prices are rejected.
