...
#[tokio::test]
async fn manual_control_remains_available_for_a_retry() {
    let dir = tempfile::tempdir().unwrap();
    let config = dir.path().join("retry.json");
    std::fs::write(&config, "{}").unwrap();
    let (mut receiver, path) = bind_manual_control(&config).await.unwrap();
    for _ in 0..2 {
        send_manual_trigger(&config).await.unwrap();
        assert_eq!(
...
#[tokio::test]
async fn nonce_lock_wait_can_be_cancelled_and_reacquired() {
    use nft_mint_bot::wallet::WalletNonceLock;
    let mut bytes = [0; 20];
    getrandom::fill(&mut bytes).unwrap();
    let wallet = alloy::primitives::Address::from(bytes);
    let held = WalletNonceLock::acquire(31337, wallet).await.unwrap();
    assert!(
        tokio::time::timeout(
            std::time::Duration::from_millis(100),
            WalletNonceLock::acquire(31337, wallet)
        )
        .await
        .is_err()
    );
...
#[test]
fn uint_arguments_must_fit_the_declared_width() {
    let mut call = MintCallConfig {
        function: "mint(uint8)".into(),
        arguments: vec!["256".into()],
        proof: None,
        price_per_nft: "0".into(),
    };
    assert!(encode_mint(&call, 1, alloy::primitives::Address::ZERO, None).is_err());
    call.arguments[0] = "255".into();
    assert!(encode_mint(&call, 1, alloy::primitives::Address::ZERO, None).is_ok());
}
