...
#[test]
fn repeated_events_preserve_the_first_confirmation_window() {
    let mut config = nft_mint_bot::config::MintConfig::load("configs/example.json").unwrap();
    config.trigger = nft_mint_bot::config::MintTrigger::ContractEvent {
        signature: "PublicSaleStarted()".into(),
        confirmations: Some(3),
    };
    let mut engine = TriggerEngine::new(&config).unwrap();
    let first = alloy::primitives::B256::repeat_byte(1);
    engine.observe_event(Some(100), Some(first), false);
    engine.observe_event(
        Some(101),
        Some(alloy::primitives::B256::repeat_byte(2)),
        false,
    );
    assert_eq!(engine.pending_event(), Some((100, Some(first))));
}

#[test]
fn view_return_types_and_targets_are_validated_before_monitoring() {
    let mut config = nft_mint_bot::config::MintConfig::load("configs/example.json").unwrap();
    for function in [
        "active()",
        "active() returns (uint256)",
        "active() returns (bool,bool)",
    ] {
        config.trigger = nft_mint_bot::config::MintTrigger::BooleanContractState {
            function: function.into(),
            expected_value: true,
        };
        assert!(config.validate().is_err());
    }
    config.trigger = nft_mint_bot::config::MintTrigger::NumericPhase {
        function: "phase() returns (uint8)".into(),
        target_value: "256".into(),
    };
    assert!(config.validate().is_err());
}
